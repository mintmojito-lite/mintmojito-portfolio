...
export const MagicTextReveal: React.FC<MagicTextRevealProps> = ({
    ...
    // Update particles with circular motion
    const updateParticles = useCallback((
        ...
        const TRANSITION_SPEED = 5 * speed;
        const FADE_SPEED = 13;
        ...
        particles.forEach(particle => {
            if (isHovered) {
                ...
                particle.floatingAngle += deltaTime * particle.floatingSpeed * 0.8;

                const radius = FLOAT_RADIUS * 0.7;
                const circleX = Math.cos(particle.floatingAngle) * radius;
                const circleY = Math.sin(particle.floatingAngle) * radius;

                const targetX = particle.originalX + circleX;
                const targetY = particle.originalY + circleY;

                const dx = targetX - particle.x;
                const dy = targetY - particle.y;

                particle.x += dx * TRANSITION_SPEED * deltaTime;
                particle.y += dy * TRANSITION_SPEED * deltaTime;
